...
class WasteConfirmScreen extends ConsumerStatefulWidget {
  const WasteConfirmScreen({super.key});

  @override
  ConsumerState<WasteConfirmScreen> createState() => _WasteConfirmScreenState();
}

class _WasteConfirmScreenState extends ConsumerState<WasteConfirmScreen> {
  ...
  void initState() {
    ...
    _recipientNameCtrl = TextEditingController(text: 'Petugas TPS');
    _recipientPhoneCtrl = TextEditingController(text: '-');
      ...
      body: Column(
        children: [
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // ── Waste summary ──
                  const Text('Detail Sampah', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600)),
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                        _InfoRow('Jenis', _categoryLabel(state.wasteCategory)),
                        _InfoRow('Ukuran', _sizeLabel(state.wasteSize)),
                        if (state.wasteWeightKg > 0)
                          _InfoRow('Berat', '${state.wasteWeightKg} kg'),
                        if (state.pickupNotes.isNotEmpty)
                          _InfoRow('Catatan', state.pickupNotes),
                      ],
                    ),
                  ),
                  ...
                  // ── Sender info (penjemput) ──
                  const Text('Info Penjemput', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600)),
                  const SizedBox(height: 10),
                  TextField(
                  ...
                  // ── Recipient info (penerima TPS — opsional) ──
                  GestureDetector(
                    onTap: () => setState(() => _showRecipient = !_showRecipient),
                    child: Row(
                      children: [
                        Icon(Icons.place_outlined, size: 20, color: ClayColors.textSecondary),
                        const SizedBox(width: 8),
                        const Text('Info Penerima di TPS (opsional)', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: ClayColors.textSecondary)),
                        const Spacer(),
                        Icon(_showRecipient ? Icons.expand_less : Icons.expand_more, color: ClayColors.textSecondary),
                      ],
                    ),
                  ),
                  if (_showRecipient) ...[
                  ...
                  _PaymentOption(
                    ...
                    onTap: () => ref.read(wasteStateProvider.notifier).setPaymentMethod('clay_wallet'),
                  ),
                  ...
                  _PaymentOption(
                    ...
                    onTap: () => ref.read(wasteStateProvider.notifier).setPaymentMethod('cash'),
                  ),
                  ...
                  if (_showPromoInput) ...[
                    ...
                    Row(
                      children: [
                        ...
                        TextButton(
                          onPressed: () {
                            ref.read(wasteStateProvider.notifier).setPromoCode(_promoController.text);
                          },
                  ...
                  // ── Fare breakdown ──
                  if (breakdown != null) ...[
                    const Text('Rincian Biaya Jemput', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600)),
                    const SizedBox(height: 10),
          ...
          Container(
            ...
            child: ClayButton(
              label: 'Pesan Penjemputan — Rp${_formatCurrency(fare)}',
  ...
  void _onConfirmOrder() {
    ...
    if (senderName.isEmpty || senderPhone.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Lengkapi info penjemput'), backgroundColor: ClayColors.error),
      );
      return;
    }

    final recipientName = _recipientNameCtrl.text.trim().isEmpty ? 'Petugas TPS' : _recipientNameCtrl.text.trim();
    final recipientPhone = _recipientPhoneCtrl.text.trim().isEmpty ? '-' : _recipientPhoneCtrl.text.trim();

    ref.read(wasteStateProvider.notifier).setSenderInfo(senderName, senderPhone);
    ref.read(wasteStateProvider.notifier).setRecipientInfo(recipientName, recipientPhone);
    ref.read(wasteStateProvider.notifier).confirmOrder();

    ref.listenManual(wasteStateProvider, (_, state) {
      if (state.orderStatus == 'finding_driver' && !state.isLoading) {
        context.go('/waste/searching');
      }
    });
  }
