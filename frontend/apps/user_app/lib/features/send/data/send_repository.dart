...
class SendRepository {
    ...
    try {
      final response = await _api.dio.post(ApiEndpoints.deliveryEstimate, data: {
        ...
        'package': {
          ...
          'size': packageSize,
          if (packageWeight > 0) 'weight_kg': packageWeight,
          if (insuranceValue > 0) 'insurance_value': insuranceValue,
        },
      });
  ...
  Future<Map<String, dynamic>> createOrder({
    ...
    try {
      ...
      final response = await _api.dio.post(ApiEndpoints.deliveryCreate, data: {
        ...
        if (destNotes.isNotEmpty) 'dest_notes': destNotes,
        ...
        'package': {
          ...
          if (packageDescription.isNotEmpty) 'description': packageDescription,
          if (insuranceValue > 0) 'insurance_value': insuranceValue,
        },
      });
  ...
  Map<String, dynamic> _mapOrderDetailResponse(Map<String, dynamic> data) {
    ...
    if (proof != null) {
      order['proof'] = {
        'pickup_photo_url': proof['pickup_photo_url'] ?? '',
        'delivery_photo_url': proof['delivery_photo_url'] ?? '',
      };
    }
